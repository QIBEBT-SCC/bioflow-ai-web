...
const StringInputCard = memo(function StringInputCard() {
    ...
    const nodeData =
        useNodesData<Node<{ args: string }, 'value_string'>>(nodeId)
...
const FileInputCard = memo(() => {
    const nodeId = useNodeId() ?? ''
    const nodeData =
        useNodesData<Node<{ args: string }, 'resource_file'>>(nodeId)
    const { updateNodeData } = useReactFlow()
...
const SequenceInputCard = memo(function SequenceInputCard() {
    ...
    useEffect(() => {
        if (
            nodeData?.data.args.r1 !== undefined &&
            nodeData.data.args.r1 !== r1
        ) {
    ...
    useEffect(() => {
        if (
            nodeData?.data.args.r2 !== undefined &&
            nodeData.data.args.r2 !== r2
        ) {
...
const DBInputCard = memo(function DBInputCard() {
    ...
    const nodeData =
        useNodesData<
            Node<{ resource_uid: string; args: string }, 'resource_db'>
        >(nodeId)
...
const ReferenceInputCard = memo(function ReferenceInputCard() {
    ...
    const nodeData =
        useNodesData<
            Node<{ args: { requiredIndex: string[] } }, 'resource_db'>
        >(nodeId)
